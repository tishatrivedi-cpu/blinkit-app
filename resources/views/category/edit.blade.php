...
<div class="d-flex align-items-center mb-3 ">
   ...
   <ol class="breadcrumb text-primary" >
       ...
       <div class="container" style=" background-color: white; padding: 2rem; border-radius: 8px; box-shadow: 0 4px 6px var(--shadow-color);width: 100%; max-width: 1000px;">
        ...
        <h2 style="margin-top: 0; color: #1A73E8; text-align: center;">Edit Category</h2>
        <form id="category-form" style="display: flex; justify-content: center; flex-direction: column; gap: 1.5rem;"  method="POST" action="{{route("category.update",$category->_id)}}" enctype="multipart/form-data" >
            @csrf
           @method('PUT')
            <div style="display: flex; flex-direction: column; gap: 0.5rem;">
                <label for="category-name" style="font-weight: 600; font-size: 0.9rem;">Category Name</label>
                <input type="text" id="category-name" name="cat_name"  value="{{$category->cat_name}}"  placeholder="Enter category name" style="width: 100%; padding: 0.75rem; border: 1px solid #e1e4e8; border-radius: 4px; font-size: 1rem; transition: border-color 0.3s ease;">
                 @error('cat_name')
                            {{$message}}
                        @enderror
            </div>
            <div style="display: flex; flex-direction: column; gap: 0.5rem;">
                <label for="category-image" style="font-weight: 600; font-size: 0.9rem;">Category Image</label>
                <div id="drop-area" style="border: 2px dashed #e1e4e8; border-radius: 4px; padding: 20px; text-align: center; transition: all 0.3s ease; cursor: pointer;">
                    ...
                    <div style="position: relative; overflow: hidden; display: inline-block; margin-top: 10px;">
                        ...
                        <input type="file" id="cat_pic" name="cat_pic"  value="{{$category->cat_pic}}"   accept="image/*"  style="position: absolute; left: 0; top: 0; opacity: 0; cursor: pointer; width: 100%; height: 100%;">
                    </div>
                </div>
            ...
            <button type="submit" style="background-color: #1A73E8; color: white; border: none; padding: 0.75rem; border-radius: 4px; font-size: 1rem; cursor: pointer; transition: background-color 0.3s ease;">Edit Category</button>
             <div class="container m-2" style="display: flex; justify-content: center; align-items: center; " >
        <a href="/category" class="btn btn-primary my-3 col-6 ">Back</a>
