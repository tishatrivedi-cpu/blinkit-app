@include('header')
   ...
   <ol class="breadcrumb text-primary" >
                ...
                <li class="breadcrumb-item"  aria-current="page">Edit-Store</li>
       ...
       <div class="container" style=" background-color: white; padding: 2rem; border-radius: 8px; box-shadow: 0 4px 6px var(--shadow-color);width: 100%; max-width: 1000px;">
        ...
        <h2 style="margin-top: 0; color: #1A73E8; text-align: center;">Edit Store</h2>
        <form id="category-form" style="display: flex; justify-content: center; flex-direction: column; gap: 1.5rem;"  method="POST" action="{{route("store.update",$store->_id)}}" enctype="multipart/form-data" >
            @csrf
            @method('PUT')
            <div style="display: flex; flex-direction: column; gap: 0.5rem;">
                <label for="category-name" style="font-weight: 600; font-size: 0.9rem;">Store Name</label>
                <input type="text" id="store-name" name="s_name" value="{{$store->s_name}}" placeholder="Enter store name" style="width: 100%; padding: 0.75rem; border: 1px solid #e1e4e8; border-radius: 4px; font-size: 1rem; transition: border-color 0.3s ease;">
            ...
            <div style="display: flex; flex-direction: column; gap: 0.5rem;">
                ...
                <div id="drop-area" style="border: 2px dashed #e1e4e8; border-radius: 4px; padding: 20px; text-align: center; transition: all 0.3s ease; cursor: pointer;">
                    ...
                    <div style="position: relative; overflow: hidden; display: inline-block; margin-top: 10px;">
                        ...
                        <input type="file" id="S_pic" name="s_pic" value="{{$store->s_pic}}" accept="image/*"  style="position: absolute; left: 0; top: 0; opacity: 0; cursor: pointer; width: 100%; height: 100%;">
                    </div>
                </div>
             ...
             <div style="display: flex; flex-direction: column; gap: 0.5rem;">
                ...
                <input type="text" id="store-name" name="s_addres" value="{{$store->s_addres}}"  placeholder="Enter store Addres" style="width: 100%; padding: 0.75rem; border: 1px solid #e1e4e8; border-radius: 4px; font-size: 1rem; transition: border-color 0.3s ease;">
                <span class="text-danger">
                        @error('s_addres')
                            {{$message}}
                        @enderror
             ...
             <div style="display: flex; flex-direction: column; gap: 0.5rem;">
                ...
                <input type="text" id="store-name" name="pincode" value="{{$store->pincode}}"  placeholder="Enter Pincode" style="width: 100%; padding: 0.75rem; border: 1px solid #e1e4e8; border-radius: 4px; font-size: 1rem; transition: border-color 0.3s ease;">
                <span class="text-danger">
                        @error('pincode')
                            {{$message}}
                        @enderror
            ...
            <div style="display: flex; flex-direction: column; gap: 0.5rem;">
                ...
                <input type="text" id="store-name" name="mobileno" value="{{$store->mobileno}}"  placeholder="Enter mobileno" style="width: 100%; padding: 0.75rem; border: 1px solid #e1e4e8; border-radius: 4px; font-size: 1rem; transition: border-color 0.3s ease;">
                <span class="text-danger">
                        @error('mobileno')
                            {{$message}}
                        @enderror
            ...
            <button type="submit" style="background-color: #1A73E8; color: white; border: none; padding: 0.75rem; border-radius: 4px; font-size: 1rem; cursor: pointer; transition: background-color 0.3s ease;">Edit Store</button>
